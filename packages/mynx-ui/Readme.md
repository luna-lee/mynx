...
![Mynx UI Logo](https://via.placeholder.com/200x80?text=Mynx+UI)
...
Mynx UI 提供了完整的 TypeScript 类型支持，可以通过以下方式使用：

### 通过导入组件时获取类型

```typescript
import { MButton } from 'mynx-ui'
import type { ButtonProps } from 'mynx-ui'
...
为了在整个项目中都能使用 Mynx UI 的全局类型，你可以在 `tsconfig.json` 中添加以下配置：

```json
{
  "compilerOptions": {
    "types": [
      "mynx-ui/client"  // 添加 Mynx UI 全局类型
    ]
  }
}
```
...
Mynx UI 在 `package.json` 中配置了多种导出方式，支持多种使用场景：
...
```json
{
  "exports": {
    ".": {
      ...
      "import": "./dist/mynx-ui.es.js",   // ESM 格式
      "require": "./dist/mynx-ui.umd.js"  // CommonJS 格式
    },
    "./client": {
...
- 导入组件库： `import { MButton } from 'mynx-ui'`
- 导入全局类型： `import 'mynx-ui/client'`
- 导入样式： `import 'mynx-ui/style.css'`
...
```bash
# 克隆仓库
git clone https://github.com/yourusername/mynx-ui.git
cd mynx-ui
