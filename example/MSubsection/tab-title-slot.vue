    ...
    >
      ...
      <template #tab-title-0="{ tab }">
        <div style="display: flex; align-items: center; gap: 6px;">
          ...
          <span>{{ tab.title }}</span>
          <span v-if="tab.badge" style="background: #ff4757; color: white; font-size: 12px; padding: 2px 6px; border-radius: 10px;">
            {{ tab.badge }}
          </span>
        </div>
      </template>
      ...
      <template #tab-title-1="{ tab }">
        <div style="display: flex; align-items: center; gap: 6px;">
          ...
          <span>{{ tab.title }}</span>
          <span v-if="tab.count !== undefined" style="background: #2ed573; color: white; font-size: 12px; padding: 2px 6px; border-radius: 50%; min-width: 20px; text-align: center;">
            {{ tab.count }}
          </span>
        </div>
      </template>
      ...
      <template #tab-title-2="{ tab }">
        <div style="display: flex; align-items: center; gap: 6px;">
          ...
          <span v-if="tab.isNew" style="background: linear-gradient(45deg, #3742fa, #2f3542); color: white; font-size: 10px; padding: 1px 4px; border-radius: 3px;">
            NEW
          </span>
        </div>
      </template>
